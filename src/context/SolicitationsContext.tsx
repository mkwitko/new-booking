...
import { IAvailVipQuery, IRequestQuery } from "@/classes/availability/DTO/AvailabilityDTO";
...
export const SolicitationsContext = React.createContext({} as SolicitationsContextProps);
      ...
      if (solicitationsHook.locator) {
        ...
        data = {
          query: {
            startDate: format(new Date(dateHook.checkIn), 'yyyy-MM-dd'),
            endDate: format(new Date(dateHook.checkOut), 'yyyy-MM-dd'),
            bookingDateType: bookingDateType[solicitationsHook.dateType as BookingDateType]
          },
          companyId: salePointHook.salePoint,
        };

        if (solicitationsHook.bookingPolicyType) {
          data.query.bookingPolicyType = bookingPolicyType[solicitationsHook.bookingPolicyType as BookingPolicyType];
        }
      }
      console.log('data - ', data)
    }
